...
/**
 * Middleware para validar propriedade de condição comercial
 * Verifica se o usuário é o fornecedor que criou a condição
 */
export const validateFornecedorOwnership = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { id_usuario } = req.user; // Do middleware de auth
    ...
    // Verificar se o usuário é o fornecedor
    ...
    // Passar a condição ao próximo middleware
